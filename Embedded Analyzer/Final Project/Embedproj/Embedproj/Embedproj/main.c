...
static float VOLTAGESAMPLE[SIZE];
...
void printSummary(){
	...
	float amplitude[10];
	float ptp[10];
	amplitude[i] = getAmpitude(VOLTAGESAMPLE,SIZE);
	amplitude[i] = getConversion(amplitude[i]);
	ptp[0] = getPeaktoPeakVoltage(VOLTAGESAMPLE,SIZE);
	float freq = 0.0;
...
void getSamples(){
	uint8_t ch = 0;
	uint8_t intsize = (uint8_t) round((1/(2*SAMPLE_RATE)/1250) * 1000000);
	for (uint8_t i = 0; i< SIZE; i++){
		VOLTAGESAMPLE[i] = adc_read(ch);
...
uint16_t adc_read(uint8_t ch)
{
	...
	// ANDing with í7? will always keep the value
	// of ëchí between 0 and 7
